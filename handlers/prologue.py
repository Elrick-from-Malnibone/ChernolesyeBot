...
PROLOGUE_PARTS = [
    ...
    "Что касается тебя, то давно уже нет у тебя иного желания, как, невзирая ни на что, вернуться под густые своды Чернолесья — твою истинную родину. Ибо ты — еще в младенчестве был похищен у любящей матери злокозненным повелителем Гномов Болотища королем Рунмабуром. Запертый в клетке, третируемый наподобие дикого зверя, ты стойко претерпевал жестокие издевательства и безмолвно сносил бесконечные унижения. Ты мечтал лишь об одном — вернуться в Проклятый Лес, единственное место на земле, где ты можешь ощутить себя в относительной безопасности. Бежать, скрыться от проклятых Гномов и злобного Рунмабура, бежать... и отомстить!",

    "Несмотря на то, что за долгие годы, проведенные в плену у Гномов Болотища, тебе не раз выпадала удобная возможность бежать, одно обстоятельство каждый раз останавливало тебя. Дело в том, что, сызмальства отрезанный от внешнего мира толстыми прутьями тюремной клетки, ты имеешь весьма условное представление о том, что поджидает тебя за пределами ненавистного Болотища, и в каком направлении тебе следует держать свой путь. Последние несколько недель обитатели Болотища находятся в состоянии войны со своими извечными врагами — Гномами соседнего Каменного Моста. Все это время в деревне не смолкает возбужденный гул — ратники без устали точат оружие, полируют шлемы и щиты, упражняются в приемах рукопашного боя.",

    "Ты чувствуешь себя настолько окрепшим, что позволяешь себе полностью расслабиться и сконцентрировать свой умственный взор на окружающих тебя гномах в попытке проникнуть в их мысли. В то же мгновение на тебя обрушивается волна смутных образов. Разочарованный тем, что не может узнать ничего полезного, ты уже отчаиваешься, как вдруг наподобие молнии воздух пронзает невидимая мысль: один из советников Рунмабура думает о встрече, которую повелитель Болотища назначил в самой глуши Чернолесья Гиллибрану — предводителю гномов Каменного Моста. Встреча должна состояться через четыре дня. Ты понимаешь, что у тебя появился шанс! Если только тебе удастся бежать, пробраться в Чернолесье и, незаметно следуя за гномьей ратью, разузнать, где именно состоятся переговоры вождей. Тогда во время встречи, когда короля не будет окружать многочисленная стража, ты сможешь попытаться убить ненавистного Рунмабура, одним ударом отомстив и ему, и всем гномам Болотища."
]

async def send_prologue(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Отправляет пролог в стриминг-режиме"""
    ...
    # 2. Постепенно добавляем остальные части
    ...
    # 3. Финал — убираем кнопку "Пропустить"
    await context.bot.edit_message_text(
        chat_id=chat_id,
        message_id=message_id,
        text=full_text
